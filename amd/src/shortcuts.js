var deps = ["jquery", "local_keyboard_shortcuts/mousetrap", "local_keyboard_shortcuts/tomloprodModal"];
define(deps, function ($, Mousetrap, TomloprodModal) {
    "use strict";
    ...
    function to_query_params(obj) {
        var str = Object.entries(obj)
            .filter(function (kv) {
                return kv[0] != null && kv[1] != null
            })
            .map(function (kv) {
                return kv[0] + "=" + kv[1]
            })
    ...
    return {
        ...
        init: function () {
            ...
            Mousetrap.bind('s r', function () {
                ...
                var id = !!get_query_params()['id'] ? get_query_params()['id'] : "1";
                var session = M.cfg.sesskey;

                if ($('body.userswitchedrole').length == 0)
                    mousetrap_goto("/course/switchrole.php" + to_query_params({
                        id: id,
                        switchrole: -1,
                        returnurl: returnurl
                    }));
            ...
            Mousetrap.bind('s e', function () {
                var id = get_query_params()['id'];
                var session = M.cfg.sesskey;
                var returnurl = encodeURI(window.location.pathname + window.location.search);
                var editing = $('body.editing').length == 1;

                var path = (id == null ? window.location.pathname : "/course/view.php");
            ...
            var commands = [{
                ...
                },
                {
                    k: 'g m',
                    s: 'User',
                    d: 'Go to Messaging page'
                },
                {
                    k: 'g c',
                    s: 'Teacher',
                    d: 'Go to cohort mnamgement page'
                },
                {
                    k: 's e',
                    s: 'Teacher',
                    d: 'Switch edition mode on/off'
                },
                {
                    k: 'g u',
                    s: 'Admin',
                    d: 'Go to user management page'
                },
                {
                    k: 'g r',
                    s: 'Admin',
                    d: 'Go to role management page'
                },
                {
                    k: 'g p',
                    s: 'Admin',
                    d: 'Go to plugin management page'
                },
                {
                    k: 's r',
                    s: 'Admin',
                    d: 'Switch role (and back)'
                },
            ];
            var strCmds = commands.map(function (cmd) {
                return "<tr><td>" + cmd.k + "</td><td>" + cmd.s + "</td><td>" + cmd.d + '</td></tr>'
            }).join('');
